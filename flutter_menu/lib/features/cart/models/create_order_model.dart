...
class CreateOrderModel extends Equatable {
  ...
  const CreateOrderModel({required this.table, required this.products, required this.userPhone, required this.userName});
  ...
  Map<String, dynamic> toJson() {
    return {
      'table': table,
      'totalPrice': products.fold(0.0, (previousValue, element) => previousValue + element.total).truncateFixed(2),
      'clientName': userName,
        ...
        for(final p in products)
          {
            ...
            'modifiers': [
              for(final m in p.modifiers.where((m) => m.selectedOptions.isNotEmpty))
